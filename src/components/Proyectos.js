  ...
  const componentesProyectos = elementosProyectos.map((elemento, index) => {
    return (
      <div className={`proyecto--${elemento.carpeta}`} key={elemento.carpeta}>
        ...
        >
          <div
            className="proyecto--descripcion"
            style={{
              boxShadow: tarjetaProyectoHov === index ? `0 0 20px ${props.modoOscuro?"white":"black"}` : "",
            }}
          >
            <p
  ...
  return (
    <section id="proyectos" className="proyectos">
      <h2 className="proyectos--titulo">
        <span className="texto--color">Proyectos: Código que Cuenta Historias</span>
      </h2>
      <p
